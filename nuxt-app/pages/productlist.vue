...
<template>
  <div class ="border-solid border-2 border-white rounded-md ">
    ...
    <UTable :rows="rows" :columns="columns" v-model:sort="sort"
      ...
      <template #rating-data="{row}">
        <div :class="{ 'bg-red-500/10': row.rating < 4.5, 'bg-green-500/10': row.rating >= 4.5 }" class="px-3s flex justify-center py-2">
          {{ row.rating }}
        </div>
      </template>
